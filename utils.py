...
def pretty_plot_confusion_matrix(MODEL_NAME, df_cm, annot=True, cmap="Oranges", fmt='.2f', fz=11,
    ...
    plt.savefig(MODEL_NAME+'/'+'Confusion matrix.png')
    plt.show()


def plot_confusion_matrix_from_data(MODEL_NAME, class_name, y_test, predictions, columns=None, annot=True, cmap="Oranges",
    ...
    columsn = columns = [i[0:4] for i in class_name]
    ...
    plot = pretty_plot_confusion_matrix(MODEL_NAME, df_cm, fz=fz, cmap=cmap, figsize=figsize, show_null_values=show_null_values, 
        pred_val_axis=pred_val_axis, lw=lw, fmt=fmt)

    
    return tp, fp, fn, df_cm
